...
class Show_Admin_Notice_Handler_Decorator {
	...
	private function handle( string $action, ...$args ): void {
		try {
			...
			( new Admin_Notice() )->set_messages( array( $e->getMessage() . ' Please check your settings to ensure you have configured & enabled content sync.' ) );

		} catch ( \Exception $e ) {
			( new Admin_Notice() )->set_messages( array( 'There was an error during Atlas Search sync', ' ' . $e->getMessage() ) );
