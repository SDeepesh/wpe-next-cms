...
use WP_Query;
use Wpe_Content_Engine\Helper\Capabilities\Capabilities;
...
class Search {
	...
	/**
	 * Check Capabilities to see if search is enabled .
	 *
	 * @access   private
	 * @var      Capabilities $settings
	 */
	 ...
	 * @param array      $types Object types to query.
	 ...
	 */
	public function search_content_engine( string $search_query, array $types, int $limit = 10, int $offset = 0, array $search_fields = null, bool $fuzzy = true, int $fuzzy_distance = 1, array $order_by ) {
		$graphql_query = <<<'GRAPHQL'
			query Search($query: String!, $types: [String], $limit: Int, $offset: Int, $fields: [String], $fuzzy: Boolean, $fuzzyDistance: Int, $orderBy: [SearchOrderBy!]) {
				search(query: $query, types: $types, limit: $limit, offset: $offset, fields: $fields, fuzzy: $fuzzy, fuzzyDistance: $fuzzyDistance, orderBy: $orderBy) {
					result
				}
			}
		GRAPHQL;
	...
	private function is_graphql_request(): bool {
		return isset( $get_query_var['graphql'] );
	}

	private function is_html_search( $post_types ): bool {
	...
	public function get_ce_posts( ?array $posts, WP_Query $query ) {
		...
		if (
			! $query->is_search ||
			! $this->capabilities->is_search_enabled() ||
			$this->is_admin_search_request() ||
			$this->is_html_search( $query_vars['post_type'] )
		) {
			$query->content_engine_search_success = false;
			return $posts;
		}

		$post_types    = array_values( ! is_array( $query_vars['post_type'] ) ? array( $query_vars['post_type'] ) : $query_vars['post_type'] );
		...
		$offset         = self::get_offset( $page_number, $posts_per_page );

		try {
			// Reach out to content engine.
			$ce_results = $this->search_content_engine( $query->query['s'], $post_types, $posts_per_page, $offset, $search_fields, $fuzzy_config['enabled'], $fuzzy_config['distance'], $order_by );
		} catch ( Exception $e ) {
