...
class Json_Schema extends Property {
	...
	private function validate(): void {
		if ( preg_match( self::ALLOWED_KEY_NAME_REGEX, $this->get_name() ) === 0 ) {
			throw   new Invalid_Json_Schema_Exception( 'Not allowed title: "' . $this->get_name() . '". Should match pattern: ' . self::ALLOWED_KEY_NAME_REGEX );
		}

		if ( ! empty( $this->object_properties ) ) {
			foreach ( $this->object_properties as $property ) {
				if ( preg_match( self::ALLOWED_KEY_NAME_REGEX, $property->get_name() ) === 0 ) {
					throw   new Invalid_Json_Schema_Exception( 'Not allowed property: "' . $property->get_name() . '". Should match pattern: ' . self::ALLOWED_KEY_NAME_REGEX );
				}
			}
		}
	}
