...
class Wpe_Content_Engine {
	...
	public function __construct( Settings_Interface $settings, Client $client ) {
		...
		// INFO: Capabilities check is not necessary. We're supporting only search for all customers.
		// Disable: $this->enable_capabilities_check().

		$this->search_config = new Search_Config();
	}

	/**
	 * Make other actions or calls needed on each page load
	 *
	 * @access   private
	 */
	private function enable_capabilities_check() {
		( new Capabilities( $this->client, $this->settings ) )->is_search_enabled();
	}
	...
	private function load_dependencies() {
		...
		*/
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'helper/constants/capabilities.php';
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'helper/capabilities/capabilities.php';
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'helper/static-site/static-site.php';
	...
	public function post_upsert_handler( int $post_id, WP_Post $post ) {
		...
		if ( ! $is_acm_model && $is_custom_post_type ) {
	...
	private function define_public_hooks() {
		...
		$search        = new Search(
			$this->client,
			$this->settings,
			new Capabilities( $this->client, $this->settings, $this->settings ),
			new Search_Config()
		);
